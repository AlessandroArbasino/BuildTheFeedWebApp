  ...
  return (
    <div className="app">
      ...
      <div className={`content ${entered ? 'content--in' : ''}`}>
        ...
        <section className="intro" aria-label="About Build The Feed">
          <div className="intro-eyebrow">How it works</div>
          <h2 className="intro-title">Turn your ideas into scheduled posts</h2>
          <p className="intro-text">
            Share any idea in natural language. We moderate it automatically, add it to the queue,
            and schedule it at the next available slot based on your daily cadence. After sending,
            you'll see exactly when it will be published.
          </p>
          <ul className="intro-steps">
            <li>Write your idea above</li>
            <li>Automatic moderation and queueing</li>
            <li>Scheduled at the next slot, shown to you instantly</li>
          </ul>
        </section>

        {toast && (
          <div className={`toast ${toastClosing ? 'toast--closing' : ''}`}>{toast}</div>
        )}

        {showModal && (
          <div className={`modal-overlay ${modalClosing ? 'modal-overlay--closing' : ''}`} role="dialog" aria-modal="true">
            <div className={`modal ${modalClosing ? 'modal--closing' : ''}`}>
              ...
              <div className="modal-actions">
                <button className="btn btn--primary" onClick={closeModal}>Great!</button>
